...
O sistema SHALL prover scripts/sync-alunos.sh que le alunos.json e executa git submodule update --remote por aluno (ou seletivo --aluno <id>), e o orientador SHALL obter a ponta dos forks com git pull + sync.

#### Scenario: Sync completo
- **WHEN** o orientador executa scripts/sync-alunos.sh
- **THEN** cada submodule avanca para a ponta do branch configurado

#### Scenario: Sync seletivo
- **WHEN** o orientador executa scripts/sync-alunos.sh --aluno joao
